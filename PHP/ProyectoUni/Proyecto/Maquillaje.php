<!DOCTYPE html>
<html lang="en">

<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <link rel="stylesheet" href="Css/Tarjetas.css">
  <title>Maquillaje</title>
  <script src="https://kit.fontawesome.com/24eb7a5550.js" crossorigin="anonymous"></script>
</head>

<body>
  <header>
    <div class="logo">
      <img src="Img/Logo1.png" alt="logo empresa">
      <h2 class="logo-nombre">Beauty Lashes</h2>
    </div>
    <nav>
    <a href="Principal.php" class="nav-link">Inicio</a>
            <a href="Pestañas.php" class="nav-link">Pestañas</a>
            <a href="Maquillaje.php" class="nav-link">Maquillaje</a>
            <a href="Cortes.php" class="nav-link">Cortes</a>
            <a href="Productos.php" class="nav-link">Productos</a>
            <a href="Agenda.php" class="nav-link">Agenda</a>
            <a href="Sesion.php" class="nav-link">Usuario</a>
    </nav>
  </header>
  <div class="Titulo">
    <h1>Maquillajes</h1>
  </div>

  <div class="product-box">
    <div class="product">
      <img src="Img/MaquillajeXV.jpg" alt="XV Años">
      <h3>XV Años</h3>
      <p>Este paquete incluye: prueba de maquillaje,prueba de peinado, maquillaje final, peinado final, manicure,pedicure y masajes.</p>
      <div class="botones">
        <a href="Agenda.php" class="add-to-cart">Llevar a agendar</a>
      </div>
    </div>

    <div class="product">
      <img src="Img/MaquillajeBoda.jpg" alt="Bodas">
      <h3>Bodas</h3>
      <p>Este paquete incluye: prueba de maquillaje,prueba de peinado, maquillaje final, peinado final, manicure,pedicure y masajes.</p>
      <div class="botones">
        <a href="Agenda.php" class="add-to-cart">Llevar a agendar</a>
      </div>
    </div>

    <div class="product">
      <img src="Img/Maquillaje EventoSocial.jpg" alt="Evento Social">
      <h3>Evento Social</h3>
      <p>Este paquete incluye: prueba de maquillaje,prueba de peinado, maquillaje final, peinado final, manicure,pedicure y masajes.</p>
      <div class="botones">
        <a href="Agenda.php" class="add-to-cart">Llevar a agendar</a>
      </div>
    </div>

    <div class="product">
      <img src="Img/MaquillajeNoche.jpg" alt="Evento Noche">
      <h3>Evento Noche</h3>
      <p>Este paquete incluye: prueba de maquillaje,prueba de peinado, maquillaje final, peinado final, manicure,pedicure y masajes.</p>
      <div class="botones">
        <a href="Agenda.php" class="add-to-cart">Llevar a agendar</a>
      </div>
    </div>

  </div>
  <footer class="pie-pagina">
    <div class="grupo-1">
        <div class="box">
            <figure>
                <a href="#">
                    <img src="Img/Logo1.png" alt="Logofooter">
                </a>
            </figure>
        </div>
        <div class="box">
            <h2>SOBRE NOSOTROS</h2>
            <p>Beauty Lashes es un moderno y acogedor salón de belleza especializado en realzar la mirada con servicios de
          extensiones de pestañas y diseño de cejas. Resalta tu belleza natural con nosotros.</p>
            
        </div>
        <div class="box">
            <h2>SIGUENOS</h2>
            <div class="red-social">
                <a href="https://www.facebook.com/"class="fa fa-facebook"></a>
                <a href="https://www.instagram.com/"class="fa fa-instagram"></a>
                <a href="https://twitter.com/home"class="fa fa-twitter"></a>
                <a href="https://www.youtube.com/"class="fa fa-youtube"></a>
            </div>
        </div>
    </div>
    <div class="grupo-2">
        <small>&copy; 2023 <b>Beauty Lashes</b> - Todos los Derechos Reservados </small>
    </div>
</footer>
</body>
</html>